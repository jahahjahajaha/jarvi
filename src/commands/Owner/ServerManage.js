...
module.exports = {
  name: "servermanage",
  category: "Owner",
  description: "Interactive panel for viewing server list or removing the bot from a server.",
  aliases: ["sm"],
  args: false,
  usage: "",
  owner: true,
  execute: async (message, args, client, prefix) => {
    // Only allowed IDs – adjust as needed
    const allowedIDs = ["1212719184870383621", "987654321012345678"];
    if (!allowedIDs.includes(message.author.id)) {
      return message.reply("❌ | Only authorized personnel can use this command.");
    }
    ...
    collector.on("end", async () => {
      const disabledRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("server_list").setLabel("Server List").setStyle(ButtonStyle.Primary).setDisabled(true),
        new ButtonBuilder().setCustomId("remove_bot").setLabel("Remove Bot").setStyle(ButtonStyle.Danger).setDisabled(true)
      );
...
async function promptResend(message, errorText, retryFunction) {
  ...
  const errorEmbed = new EmbedBuilder().setColor("Red").setDescription(errorText);
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("resend").setLabel("Resend").setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId("exit").setLabel("Exit").setStyle(ButtonStyle.Danger)
  );
  const promptMsg = await message.reply({ embeds: [errorEmbed], components: [row] });
  const collector = message.channel.createMessageComponentCollector({
    filter: i => i.user.id === message.author.id,
    max: 1,
    time: 60000
  ...
  collector.on("collect", async (intr) => {
    ...
    if (intr.customId === "resend") {
      ...
      collector.stop();
      const disabledRow = new ActionRowBuilder().addComponents(
        new ButtonBuilder().setCustomId("resend").setLabel("Resend").setStyle(ButtonStyle.Primary).setDisabled(true),
        new ButtonBuilder().setCustomId("exit").setLabel("Exit").setStyle(ButtonStyle.Danger).setDisabled(true)
      );
